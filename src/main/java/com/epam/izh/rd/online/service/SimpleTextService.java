...
public class SimpleTextService implements TextService {
    ...
    public String toJumpCase(String text) {
        ...
        for (int i = 0; i < textChars.length; i++) {
            if (i % 2 == 0) {
                ...
                sb.append(String.valueOf(textChars[i]).toUpperCase());
            }
        }
        return sb.toString();
    }
    ...
    public boolean isPalindrome(String string) {
        ...
        String buffer = string.toLowerCase().replaceAll(" ", "").trim();
        char[] textChars = buffer.toCharArray();
        for (int i = 0; i <= textChars.length / 2; i++) {
            if (textChars[i] != textChars[textChars.length - i - 1]) {
                palindrome = false;
                break;
            }
        }
